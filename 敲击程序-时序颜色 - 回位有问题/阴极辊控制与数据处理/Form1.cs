...
namespace 阴极辊控制与数据处理
{
    ...
    public partial class Form1 : UIForm
    {
        ...
        // 新增：按钮回位控制（点击按钮 → 标记目标 → 采集线程执行）
        private bool _needMoveToPosition = false;
        private int _targetMuxian = 0;
        ...
        private void uiButton7_Click(object sender, EventArgs e)
        {
            ...
            _needMoveToPosition = false; // 新增：启动时清除回位标记
            ...
            for (int row = 0; row < rows; row++)
            {
                for (int col = 0; col < cols; col++)
                {
                    ...
                    btn.FillColor = Color.Orange;      // 未采集 = 黄色/橙色
                    ...
                    // 👇👇👇 新增：按钮点击 → 仅设置回位目标（交给采集线程执行）
                    int currentRow = row;
                    int currentCol = col;
                    btn.Click += (s, ev) =>
                    {
                        int targetM = currentRow + 1;
                        int targetP = currentCol + 1;

                        if (_collectThread != null && _collectThread.IsAlive)
                        {
                            // 正在采集中 → 标记回位
                            _targetMuxian = targetM;
                            _targetPoint = targetP;
                            _needMoveToPosition = true;
                            Log($"📌 已选择回位：母线 {targetM} → 采集点 {targetP}，等待执行...");
                        }
                        else
                        {
                            // 已停止 → 直接独立回位
                            MoveToPositionDirect(targetM, targetP);
                        }
                    };
                }
            }
        ...
        private void AutoCollectLogic(int muxian, int totalLength, int step, int caijidian, int rotateAngle, string baseAudioFolder, CancellationToken token)
        {
            ...
            for (int i = 0; i < muxian; i++)
            {
                ...
                // ===================== 【回位逻辑：支持停止中断】 =====================
                if (_needMoveToPosition)
                {
                    Log($"==============================================");
                    Log($"开始执行回位：母线 {_targetMuxian} → 采集点 {_targetPoint}");
                    Log($"==============================================");
                    ...
                    try
                    {
                        ...
                        SleepWithCancel(50, token);

                        // 停止运行
                        modbus.WriteInt32(1, 73, 0);
                        SleepWithCancel(100, token);

                        // 旋转到目标母线
                        int targetRotate = (_targetMuxian - 1) * rotateAngle;

                        Log($"旋转 → 母线 {_targetMuxian}，角度 {targetRotate}");
                        modbus.WriteInt32(1, 621, targetRotate);
                        ...
                        do
                        {
                            token.ThrowIfCancellationRequested();
                            if (_stopNow) return;
                            rotateOk = ReadInt32WithCancel(1, 26, token) == 1;
                            SleepWithCancel(100, token);
                        } while (!rotateOk);

                        Log("✅ 旋转到位");
                        SleepWithCancel(1000, token);

                        // 移动到目标采集点
                        double stepDouble = (double)totalLength / (caijidian - 1);
                        int targetPosMM = (int)Math.Round((_targetPoint - 1) * stepDouble);

                        Log($"移动 → 采集点 {_targetPoint}，位置 {targetPosMM} mm");
                        modbus.WriteInt32(1, 618, targetPosMM);
                        modbus.WriteInt32(1, 69, 1);
                        SleepWithCancel(50, token);
                        modbus.WriteInt32(1, 69, 0);
                        ...
                        do
                        {
                            token.ThrowIfCancellationRequested();
                            if (_stopNow) return;
                            moveOk = ReadInt32WithCancel(1, 27, token) == 1;
                            SleepWithCancel(100, token);
                        } while (!moveOk);
                ...
                for (int ii = 0; ii < caijidian; ii++)
                {
                    ...
                    this.Invoke((Action)(() =>
                    {
                        ...
                        if (buttonIndex >= 0 && buttonIndex < _dynamicButtons.Count)
                        {
                            ...
                            btn.FillHoverColor = Color.Green;
                            btn.FillPressColor = Color.DarkGreen;
                        }
                    }));
                    // ======================
                    // 2. 不是最后一个点才移动
        ...
        // 新增：停止后也能直接回位（独立线程）
        private async void MoveToPositionDirect(int targetMuxian, int targetPoint)
        {
            ...
            _collectThread = new Thread(() =>
            {
                ...
                {
                    Log($"==============================================");
                    Log($"开始回位：母线 {targetMuxian} → 采集点 {targetPoint}");
                    Log($"==============================================");
                    ...
                    // 旋转
                    int muxianTotal = int.Parse(uiTextBox4.Text);
                    int rotateAngle = 360 / muxianTotal;
                    int targetRot = (targetMuxian - 1) * rotateAngle;

                    Log($"旋转 → 母线 {targetMuxian}，角度 {targetRot}");
                    modbus.WriteInt32(1, 621, targetRot);
                    modbus.WriteInt32(1, 68, 1);
                    SleepWithCancel(50, token);
                    modbus.WriteInt32(1, 68, 0);
                    ...
                    // 平移
                    int totalLen = int.Parse(uiTextBox6.Text);
                    int points = int.Parse(uiTextBox5.Text);
                    double step = (double)totalLen / (points - 1);
                    int targetPos = (int)Math.Round((targetPoint - 1) * step);

                    Log($"移动 → 采集点 {targetPoint}，位置 {targetPos} mm");
                    modbus.WriteInt32(1, 618, targetPos);
                    modbus.WriteInt32(1, 69, 1);
                    SleepWithCancel(50, token);
                    modbus.WriteInt32(1, 69, 0);
